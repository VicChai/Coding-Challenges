...
function balancedNum(number){
    ...
    let slice2 = []
    if(arr.length % 2 === 0 ){
      slice1 = arr.slice(0,arr.length/2-1)
      slice2 = arr.slice(arr.length/2+1)
    } else {
      slice1 = arr.slice(0,arr.length/2)
      slice2 = arr.slice(arr.length/2+1)
    }
  return slice1.reduce((a,b)=>+a+ +b,0) === slice2.reduce((a,b)=>+a+ +b,0) ? "Balanced" : "Not Balanced"
}